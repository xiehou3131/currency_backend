...
def test(request):
    username = "xiehou_1"
    result = myauths.find({"username": username})
    user = list(result)[0]
    _id = user["_id"]
    # print(_id)
    print(time_now())
    return json_wrap({"status": 200, "data": _id}, no_response=True)

@check_login
@check_parameters(["invest_cycle","invest_amount"])
def newInvestPlan(request):
    create_time = timestamp_now()
    start_time = (int(create_time / DAY) + 1) * DAY
    invest_cycle = int(request.POST['invest_cycle']) * DAY
    invest_amount = int(request.POST['invest_amount'])

    plan = {"username": request.session["username"],
